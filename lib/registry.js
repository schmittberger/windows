...
function registry(key, options){
  ...
  registry.query(p).reduce(function(current, line){
	...
	//if accessing remote registries we need to remove the host part
	if(key.startsWith("\\\\")) {
		key = key.slice((key.slice(2).indexOf("\\")+3));
	}

    if (line.indexOf(key) === 0 && line !== key) {
      // all registry keysets report their full path, will start with the initial query
